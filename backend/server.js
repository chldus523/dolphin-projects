...
app.use(cors({
    origin: [
        ...
        'http://127.0.0.1:5500',
        'http://localhost:5500',
        'https://dolphin-projects-production.up.railway.app'
    ],
    credentials: true
}));
...
function buildSystemPrompt(filteredPolicies, profile = null, matchInfo = {}) {
    ...
    return `
[언어 규칙 - 절대 위반 금지]
반드시 한국어로만 답변한다. 영어, 중국어(한자), 일본어, 러시아어 등 다른 언어는 단 한 글자도 사용하지 않는다.
외국어가 섞이면 즉시 한국어로 재작성한다.
...
의료, 법률, 정신건강 위기 상담처럼 전문가의 판단이 필요한 사안이라도, 데이터베이스에 있는 관련 정책(건강·심리 카테고리 등)이 있다면 반드시 먼저 안내한다. 정책 안내를 생략하고 전문기관 연락처만 주는 것은 금지한다.
자살, 자해, "죽고 싶다" 등 명백한 위기 신호가 보일 때만 전문 기관(정신건강위기상담전화 1577-0199, 보건복지상담센터 129 등)을 정책 안내와 함께(대체가 아니라 추가로) 안내한다. 단순히 "심리 상담 받고 싶어요"처럼 위기 신호 없는 일반적인 요청에는 위기상담전화만 던지지 말고, 반드시 건강·심리 카테고리의 실제 지원 프로그램부터 소개한다.
`.trim();
}
